...
class Spreadsheet_Excel_Reader {
	...
	function val($row,$col,$sheet=0) {
		$col = $this->getCol($col);
		// echo $col. "<br/>";
		// print_r($this->sheets[$sheet]['cells']);
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
						...
						$xf['borderTop'] = isset($this->lineStyles[($border & 0xF00) >> 8]) ? $this->lineStyles[($border & 0xF00) >> 8] : 0;
	...
	function _parsesheet($spos) {
		...
		function _getCellDetails($spos,$numValue,$column) {
			...
			if ($type == 'date') {
				...
				$utcDays = floor($numValue - SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS); /*($this->nineteenFour ? SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS1904 : SPREADSHEET_EXCEL_READER_UTCOFFSETDAYS));*/
