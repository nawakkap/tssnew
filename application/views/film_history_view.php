<?= form_open("/film/history") ?>
<table border="0" width="100%" class="ui-widget">
	<tr>
		<td align="left">
			<input type="button" id="backButton" name="backButton" class="button" value="ย้อนกลับ" />
			<input type="button" id="filterButton" name="filterButton" class="date_button" value="<?= $startDate ?> / <?= $endDate ?>" onclick="onDateFilterClick()" />
		</td>
		<td align="right">
			<?
...
<table border="1" width="100%" class="table-data ui-widget">
	...
	<tbody>
		...
		<tr class="odd">
			...
			<td><a href="#" class="link" onclick="onDateClick('<?= mysqldatetime_to_date($item['slit_date'], 'Y-m-d') ?>')"><?= mysqldatetime_to_date($item['slit_date'], 'd/m/Y') ?></a></td>
...
<table border="0" width="100%">
	<tr>
		<td align="right"><input type="button" name="mainButton" class="button mainButton" value="ย้อนกลับหน้าแรก"/></td>
	</tr>
</table>
<?= form_open("/film/history", array("id" => "sort_form")) ?>
...
<?= form_open("film/history", array("id" => "FilterByDateForm")) ?>
