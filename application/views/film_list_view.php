...
<table border="0" width="100%" class="ui-widget">
	<tr>
		...
		<td align="right">
		<!--
			<input type="button" name="editButton" value="แก้ไข" class="button" onclick="onEditClick()" />
			<input type="button" name="saveButton" value="บันทึก" class="button" onclick="onSaveClick()" />
		-->
		</td>
	</tr>
</table>
...
<table border="1" width="100%" cellpadding="3" class="table-data ui-widget">
	<thead>
		<tr class="ui-widget-header">
			...
			<th style="background-color: yellow" align="center">&nbsp;</th>
			<th align="center" nowrap="nowrap"><a href="#" class="sort" rel="unit"># แถบ</a></th>
			<th align="center" nowrap="nowrap"><a href="#" class="sort" rel="expanded"># ยืด</a></th>
			<th align="center" nowrap="nowrap"><a href="#" class="sort" rel="total_quantity"># ที่ผลิต</a></th>
			<th align="center" nowrap="nowrap"><a href="#" class="sort" rel="closed"># ปิดล๊อต</a></th>
	...
	<tbody>
		...
		<tr class="<?= ($disabled) ? "limit" : "odd" ?>">
			...
			<td><a class="link" href="<?php echo site_url("film/item_detail/" . $this->convert->AsciiToHex($item['coil_group_code']) . "/" . $this->convert->AsciiToHex($item['product_dtl_id'])); ?>"><?= number_format($item['unit'], 0) ?></a></td>
			<td><?= number_format($item['expanded'], 0)  ?></td>
			<td><?= number_format($item['total_quantity'], 0)  ?></td>
			<td><?= number_format($item['closed'], 0)  ?></td>
...
<input type="hidden" name="theDate" value="<?= $theDate ?>"/>
<?= form_close() ?>
<?= form_open("/film/edit_film", array("id" => "edit_film")) ?>
<input type="hidden" name="date" value="<?= $theDate ?>" />
<?= form_close() ?>
