	...
	<tbody>
		...
		<tr class="odd">
			...
			<td align="center">
				...
				<input type="text" name="quantity_<?= $suffix ?>" class="quantity numeric" value="<?= ($item["quantity"]) ? $item["quantity"] : 0 ?>" size="5" max_unit="<?= number_format($item["unit"]) ?>" rel="<?= number_format($item['total_quantity'], 0) ?>" today_unit="<?= ($item["quantity"]) ? $item["quantity"] : 0 ?>" <?= $disabled ?> />
			</td>
			<td>
			<? if ($has_ironing) { ?>
				<!-- Has Ironing -->
			...
			<? } else { ?>
			<select name="machine_<?= $suffix ?>" class="machine" rel="<?= $item["program_code"] ?>">
				<option value=""></option>
				<? for($i = 0; $i < count($machine); $i++) {
					$checked = '';
					if ($machine[$i]["mc_id"] == $item["mc_id"]) {
						$checked = 'selected="selected"';
					}
				?>
				<option value="<?= $machine[$i]["mc_id"] ?>" <?= $checked ?>><?= $machine[$i]["machine_name"] ?></option>
				<? } ?>
			</select>
